...
def random_forest(X_train, X_test, y_train, y_test):
    params = {
        'n_estimators': [500],
        'max_features': [3,5,7,9],#[5],
        'max_depth': [135,140,142],#[140],
    }
    model = GridSearchCV(estimator=RandomForestClassifier(max_depth=None), param_grid=params, cv=10)
    model=RandomForestClassifier(max_depth=None, n_estimators=500,max_features=5)
...
def decision_tree(X_train, X_test, y_train, y_test):
    params = {
        'max_depth':  [1,2,3,4,5,6,10,15,20,60,None]#[1], #
    }
    model = GridSearchCV(estimator=DecisionTreeClassifier(), param_grid=params, cv=10)
    model.fit(X_train, y_train)
    print('----------Decision Tree----------------')
    print('best params:')
    print(model.best_params_)
...
def NN(X_train, X_test, y_train, y_test):
    params = {
        'hidden_layer_sizes': [(100), (50,50), (20,20,20)], # [(100)],#
...
X_train, X_test, y_train, y_test = readFile(path=path, y_label=y_label, encode_features=encode_features, training_ratio=0.7,method='UnderSample',shuffle=False)
