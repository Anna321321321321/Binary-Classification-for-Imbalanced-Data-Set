import numpy as np
from fileIO_weight import readFile
...
def random_forest(X_train, X_test, y_train, y_test):
    ...
    model=RandomForestClassifier(max_depth=None, n_estimators=500,max_features=5,class_weight={0: 1, 1: 3}) #, {0: 1, 1: 5)}class_weight={0: 1, 1: 5}
...
def decision_tree(X_train, X_test, y_train, y_test):
    params = {
        ...
        'class_weight':[{0: 1, 1: 0.5},{0: 1, 1: 3},{0: 1, 1: 1},{0: 1, 1: 2},{0: 1, 1: 3}]
    }
...
def SVM(X_train, X_test, y_train, y_test):
    params = {
        ...
        'kernel': ['rbf'],
        'class_weight':[{0:1,1:0.5},{0:1,1:1},{0:1,1:2},{0:1,1:3}],
    }
    model = GridSearchCV(estimator=SVC(gamma='scale'), param_grid=params, cv=10)
    # model = SVC(gamma='scale', kernel='rbf',class_weight={1: 5})
    model.fit(X_train, y_train)
...
def logistic_regression(X_train, X_test, y_train, y_test):
    params = {
        ...
        'class_weight':[{0:1,1:0.5},{0:1,1:1}],#,{0:3,1:1},{0:5,1:1}]
    }
...
def NN(X_train, X_test, y_train, y_test):
    params = {
        ...
        # 'sample_weight':[{0:1,1:0.5},{0:1,1:1}],
...
X_train, X_test, y_train, y_test = readFile(path=path, y_label=y_label, encode_features=encode_features, training_ratio=0.7,method='None')
